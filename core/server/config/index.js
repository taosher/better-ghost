...
_private.loadNconf = function loadNconf(options) {
    ...
    const overrideFromEnvs = (env, key) => {
        process.env[env] && nconf.set(key, process.env[env]);
    };
  ...
  */
    ...
    overrideFromEnvs('GHOST_INSTALL', 'paths:appRoot');
    overrideFromEnvs('GHOST_CONTENT', 'paths:contentPath');
    overrideFromEnvs('GHOST_URL', 'url');
    overrideFromEnvs('GHOST_HOST', 'server:host');
    overrideFromEnvs('GHOST_PORT', 'server:port');
    overrideFromEnvs('GHOST_DB_PATH', 'database:connection:filename');
